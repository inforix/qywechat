...
const crypto = require('crypto'), //引入加密模块
    ...
    querystring = require('querystring'), //引入 querystring 模块
...
var wxCrypto = require('./wechat-crypto.js');
...
var WeChat = function (config) {
     ...
     * 用于处理 https Post请求方法
     * @param {String} url  请求地址
     * @param {JSON} jsonData 提交的数据
     */
    this.requestPost = function (url, jsonData) {
    ...
    this.getAuthRedirectUrl = function() {
        var url = 'https://open.weixin.qq.com/connect/oauth2/authorize?';
        var info = {
            appid: this.appid,
            redirect_uri: 'https://dk.shmtu.edu.cn/wechat-auth',
            response_type: 'code',
        ...
        url = url + querystring.stringify(info) + '#wechat_redirect';
        return url;
    }
}
...
WeChat.prototype.auth = function (req, res) {
    ...
    var array = [this.token, timestamp, nonce, echostr];
    array.sort();
    ...
    if (resultCode === signature) {
        var ret = this.cryptor.decrypt(echostr);
        console.log(ret);
        res.send(ret.message); /*
        XmlParse(ret.message, function(err, json){
            console.log(json);
            var msg = json.msg;
            res.send(msg);
        })*/
...
WeChat.prototype.getAccessToken = function () {
    ...
    return new Promise(function (resolve, reject) {
        ...
        if (that.accessToken.access_token === "" || that.accessToken.expires_time < currentTime) {
            //格式化请求地址
            var url = util.format(that.apiURL.getAccessToken, that.appid, that.appScrect);
            that.requestGet(url).then(function (data) {
                var json = JSON.parse(data);
                if (json.errcode === 0) {
                    that.accessToken.access_token = json.access_token;
                    that.accessToken.expires_time = new Date().getTime() + (parseInt(json.expires_in) - 200) * 1000;
                    //将获取后的 access_token 返回
                    resolve(that.accessToken.access_token);
                } else {
                    //将错误返回
                    reject(json);
                }
...
/** 
 * 获取用户信息
 * @param code 从微信调用返回的Code
 * @returns JSON object, {"UserId":"USERID", "DeviceId": "DEVICEID"}
 */
WeChat.prototype.getUserInfo = function(code){
    var that = this;

    return new Promise(function (resolve, reject) {
        that.getAccessToken().then(function(token){
            // aadd code to access openid
            let url = 'https://qyapi.weixin.qq.com/cgi-bin/user/getuserinfo?' + querystring.stringify({
                access_token: token,
                code: code,
            });
...
WeChat.prototype.sendText = function (userid, text) {
    var that = this;

    return new Promise(function (resolve, reject){
        that.getAccessToken().then(token=>{
            let url = 'https://qyapi.weixin.qq.com/cgi-bin/message/send?' + querystring.stringify({access_token:token});
            let data = {
              "touser": userid,
              "msgtype" : "text",
            ...
            that.requestPost(url, data).then(ret=>{
...
WeChat.prototype.parseXmlToJSON = function(xml){
 ...
 * @param {Response} res Response 对象
 */
WeChat.prototype.handleMsg = function (req, res, locationCallback) {
    var buffer = [],
        that = this;
        ...
        parseString(msgXml, {
            ...
            if (!err) {
                ...
                //if (req.query.encrypt_type == 'aes') {
                    ...
                    result = that.cryptor.decrypt(result.Encrypt);
                    result = that.parseXmlToJSON(result.message);
                //}
